...
class BookingTestCase(unittest.TestCase):
    ...
    def test_booking_displaying_clubs(self, mock_clubs):
        ...
        self.other_clubs_number = len(
            [c for c in mock_clubs if c != self.selected_club]
            )
        ...
        first_test_club_name = (
            f"{self.first_test_club['name']}"
            )
        first_test_club_points = (
            f"Points available: {self.first_test_club['points']}"
            )
        second_test_club_name = f"{self.second_test_club['name']}"
        second_test_club_points = (
            f"Points available: {self.second_test_club['points']}"
            )
        ...
        number_of_club_items_rendered = response.data.count(
            b'class="club-item"'
        )
        self.assertEqual(
            number_of_club_items_rendered,
            self.other_clubs_number
        )
