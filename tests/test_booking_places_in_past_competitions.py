...
class BookingTestCase(unittest.TestCase):
    ...
    @patch('server.competitions', [
        {'name': 'Test competition', 'date': (datetime.now() + timedelta(days=1)).strftime('%Y-%m-%d %H:%M:%S'), 'numberOfPlaces': 9}
    ])
    @patch('server.clubs', [
    ...
    @patch('server.competitions', [
        {'name': 'Test competition', 'date': (datetime.now() + timedelta(minutes=1)).strftime('%Y-%m-%d %H:%M:%S'), 'numberOfPlaces': 9}
    ])
    @patch('server.clubs', [
        {'name': 'Test club', 'email': 'unittest@email.com', 'points': 12}
    ])
