...
class TestBookingSystem(unittest.TestCase):
    ...
    @patch('server.competitions', new_callable=lambda: [
        {'name': 'Competition One', 'date': '2024-12-01 10:00:00', 'numberOfPlaces': 20},
        {'name': 'Competition Two', 'date': '2024-12-02 10:00:00', 'numberOfPlaces': 15}
    ])
    def test_booking_with_multiple_cases(self, mock_competitions, mock_clubs):
        test_cases = [
            (mock_clubs[0]['name'], mock_competitions[0]['name'], 5, f"Points available: {mock_clubs[0]['points'] - 5}"),
            (mock_clubs[1]['name'], mock_competitions[1]['name'], 2, f"Points available: {mock_clubs[1]['points'] - 2}"),
            (mock_clubs[2]['name'], mock_competitions[0]['name'], 7, f"Points available: {mock_clubs[2]['points'] - 7}"),
        ]
